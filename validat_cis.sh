...
test_1.3.2() {
    ...
    ## Tests Start ##
    [ $(grep -r aide /etc/cron.* /etc/crontab 2>/dev/null | wc -l) -ne 0 ] && result="Pass"
    ## Tests End ##
...
test_1.4.2() {
    ...
    [ $(grep "^set superusers" /boot/grub/grub.cfg | wc -l) -ne 0 ] && [ $(grep "^password" /boot/grub/grub.cfg | wc -l) -ne 0 ] && state=0
    [ $state -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_1.5.4() {
    ...
    state=0
    str='ExecStart=-/bin/sh -c "/usr/sbin/sulogin; /usr/bin/systemctl --fail --no-block default"'
    [ $(dpkg -s prelink &>/dev/null; echo $?) -eq 1 ] && result="Pass"
    ## Tests End ##
...
test_1.6.1.3() {
    ...
    ## Tests Start ##
    state=0
    [ $(dpkg -s apparmor &>/dev/null; echo $?) -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_1.8() {
    ...
    ## Tests Start ##
    [ $(apt-get -s upgrade &>/dev/null; echo $?) -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_3.4.1() {
    ...
    ## Tests Start ##
    [ $(dpkg -s tcpd &>/dev/null; echo $?) -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_4.2.3() {
    ...
    ## Tests Start ##
    [ $(dpkg -s rsyslog &>/dev/null; echo $?) -eq 0 ] && result="Pass"
    ## Tests End ##
...
test_5.3.3() {
    ...
    pwauth_history=$(egrep '^password\s+required\s+pam_pwhistory.so' /etc/pam.d/common-password)
    ...
    [ $pwauth_history_count -ge 5 ] && state=0
...
if [ $(is_test_included 1; echo $?) -eq 0 ]; then   write_cache "1,Initial Setup"
    ...
    if [ $(is_test_included 1.1; echo $?) -eq 0 ]; then   write_cache "1.1,Filesystem Configuration"
        ...
        if [ $(is_test_included 1.1.1; echo $?) -eq 0 ]; then   write_cache "1.1.1,Disable unused filesystems"
            ...
            run_test 1.1.1.6 1 test_1.1.1.x udf   ## Ensure mounting of udf is disabled
        fi
    ...
    if [ $(is_test_included 1.4; echo $?) -eq 0 ]; then   write_cache "1.4,Secure Boot Settings"
        run_test 1.4.1 1 test_perms 400 /boot/grub/grub.cfg   ## 1.4.1 Ensure permissions on bootloader config are configured
...
if [ $(is_test_included 5; echo $?) -eq 0 ]; then   write_cache "5,Access Authentication and Authorization"
    if [ $(is_test_included 5.1; echo $?) -eq 0 ]; then   write_cache "5.1,Configure cron"
        run_test 5.1.1 1 test_is_enabled cron "cron daemon"   ## 5.1.1 Ensure cron daemon is enabled (Scored)
